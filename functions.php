...
function era_ai_scripts()
{
	...
	// core css (global + tokens + typography + button + navbar bundled into one request)
	wp_enqueue_style(
		'eraai-core',
...
/**
 * Performance Optimizations - Clean up wp_head and disable emojis
 */
function eraai_optimize_head_cleanup() {
	...
	add_filter( 'tiny_mce_plugins', 'eraai_disable_emojis_tinymce' );
	add_filter( 'wp_resource_hints', 'eraai_disable_emojis_remove_dns_prefetch', 10, 2 );

	// Remove other unnecessary header links
...
add_action( 'init', 'eraai_optimize_head_cleanup' );

/**
 * Filter function to remove tinymce emoji plugin
 */
function eraai_disable_emojis_tinymce( $plugins ) {
	if ( is_array( $plugins ) ) {
 ...
 */
function eraai_disable_emojis_remove_dns_prefetch( $urls, $relation_type ) {
